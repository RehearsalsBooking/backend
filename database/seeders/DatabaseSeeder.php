...
class DatabaseSeeder extends Seeder
{
    ...
    protected function createIndividualRehearsals(int $count): void
    {
        foreach (range(1, $count) as $_) {
            try {
                ...
                (new RehearsalTimeValidator())->validate(new RehearsalData(
            ...
            PDOException|
            QueryException|
            TimeIsUnavailableForUsersException|
            UserHasAnotherRehearsalAtThatTimeException|
            TooLongRehearsalException|
            TimeIsUnavailableInRoomException|
            InvalidRehearsalDurationException
            ) {
    ...
    private function createBandsForLoggedInUser(): void
    {
        foreach (range(1, 2) as $_) {
            ...
            $this->users
                ->random(self::BAND_MEMBERS_COUNT)
                ->where('id', '!=', $this->userToLoginWith->id)
                ->pluck('id')
                ->each(
